...
for (nameValueFlag in nameValueList) {
    ...
    colListD=colList
    x=sort(unique(annColAll$disOntTerm2))
    colListD[which(x=="pancreas")]="magenta"
    colListD[which(x=="colon")]="purple"
    ...
    if (clusterFlag[1]=="_supervised") {
        ...
        fName1=""
        datadirG=paste("results/ucsf500/",sub("_","",subsetFlag[1]),"/geneSampleId/",sep="")
        genesetList=dir(datadirG,pattern="geneSampleId_geneByDisease"); genesetList=genesetList[grep(tolower(nameValue$value[which(nameValue$name=="subset")]),tolower(genesetList))]; genesetList=sub(".RData","",sub("geneSampleId_","",genesetList),fixed=T)
        subset2Flag=c("")
        subset3Flag[1]=""
        geneFamilyFlag=F
        ...
        subsetFFlag="_men1"
        subsetFFlag=""
        load(file=paste(datadirG,"geneList.RData",sep=""))
        ...
        load(file=paste(datadirG,"geneList",geneset2Flag,subsetFlag[2],".RData",sep=""))
                ...
                for (genesetFlag in genesetList) {
                    ...
                    for (geneset2Flag in geneset2List) {
                        ...
                        if (clusterFlag[1]=="_supervised") {
                            ...
                            if (subsetFlag[1]%in%c("_ucsf500","_ucsf500Fmi")) varList=c("diffEB","ki67EB2","cellSizeEB")
                            #if (subsetFlag[1]%in%c("_ucsf500")) varList=c("diffEB","ki67EB2","cellSize")
                            if (!subsetFlag[1]%in%c("_ucsf500","_ucsf500Fmi")) {
                                if (exists("clin32")) {
                            ...
                            k=match(c("fionaCol1","tmbLevel","ki67EB2"),varName); k2=which(!is.na(k)); k1=k[k2]
                            varName[k1]=c("vetted","TMB","ki67EB")[k2]
                        ...
                        if ((length(clusterFlag)==1 & clusterFlag[1]%in%c("_supervised")) | (clusterFlag[1]%in%c("_comutated") & sampleBar=="")) {
                            ...
                            if (length(varId)==1) {
                                ...
                                varId=1
                                if (any(c("disease","disOntTerm","disOntTerm2","disOntTerm3")%in%varList[varId])) {
                        ...
                        if (is.null(varList)) {
                            ...
                            for (varId in 1:length(varList)) {
                                if (varList[varId]%in%c("sd","ki67EB2")) {
                                    ...
                                    if (varList[varId]%in%c("disOntTerm2")) {
                        ...
                        if (datTypeFlag[2]%in%c("_patient")) {
                            ...
                            if (subsetFlag[1]%in%c("_ucsf500","_ucsf500Fmi")) {
                                ...
                                varList2=c("sex","primarySite","diffEB","cellSizeEB","anyAlt")
